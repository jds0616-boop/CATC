/* --- admin.js (Final Version: Unified & Optimized) --- */
...
const dataMgr = {
    ...
    forceEnterRoom: async function(room) {
        ...
        dbRef.connections.on('value', s => {
            document.getElementById('currentJoinCount').innerText = s.numChildren();
        });
    ...
    saveSettings: function() {
        ...
        if (statusVal === 'active') {
            ...
            ui.showAlert(`✅ [Room ${state.room}] 강의 종료 처리되었습니다.`); 
        }
    },
    deactivateAllRooms: async function() {
        if(!confirm("⚠️ 모든 강의실을 '비어있음' 상태로 변경하시겠습니까?")) return;
        const updates = {};
        for(let i=65; i<=90; i++) {
    ...
    initRoomSelect: function() {
        firebase.database().ref('courses').on('value', s => {
            ...
            for(let i=65; i<=90; i++) {
                ...
                opt.value = c;
                if(st.roomStatus === 'active') opt.innerText = `Room ${c} (🔴 사용중, ${userCount}명)`;
...
const quizMgr = {
        ...
        r.onload = (evt) => {
            ...
            b.forEach(bl => {
                ...
                if (l.length >= 2) {
                    ...
                    state.quizList.push({ text: l[0], options: l.slice(1, l.length - 1), correct: parseInt(lastLine) || 0, checked: true });
                }
            });
    ...
    renderMiniList: function() {
        ...
        state.quizList.forEach((q, i) => {
            d.innerHTML += `<div style="padding:5px; border-bottom:1px solid #eee; font-size:11px;">Q${i+1}. ${q.text.substring(0,15)}...</div>`;
        });
    },
    showQuiz: function() {
        const q = state.quizList[state.currentQuizIdx];
        document.getElementById('d-qtext').innerText = q.text;
        const oDiv = document.getElementById('d-options'); oDiv.innerHTML = "";
        q.options.forEach((o, i) => { oDiv.innerHTML += `<div class="quiz-opt">${i+1}. ${o}</div>`; });
    ...
    prevNext: function(d) { state.currentQuizIdx += d; this.showQuiz(); }
};

// --- 5. Print & Report (최종 고도화 버전) ---
const printMgr = {
    ...
    confirmPrint: function() { 
        ...
        const date = document.getElementById('printDateInput').value || "2026.01.07";
            ...
            items.forEach((item, idx) => {
                ...
                listBody.innerHTML += `
                    <tr>
                        ...
                        <td style="text-align:center;">${item.status || '일반'}</td>
                    </tr>`;
            });
        }
